...
export function line(x1, y1, x2, y2, o = {}) {
  return `<line x1="${n(x1)}" y1="${n(y1)}" x2="${n(x2)}" y2="${n(y2)}" stroke="${o.stroke || RULE}" stroke-width="${o.width || 1}"${o.dash ? ` stroke-dasharray="${o.dash}"` : ''}${o.opacity !== undefined ? ` opacity="${o.opacity}"` : ''} />`
}

export function circle(cx, cy, r, o = {}) {
  return `<circle cx="${n(cx)}" cy="${n(cy)}" r="${n(r)}" fill="${o.fill || 'none'}"${o.opacity !== undefined ? ` opacity="${o.opacity}"` : ''}${o.stroke ? ` stroke="${o.stroke}" stroke-width="${o.width || 1}"` : ''} />`
}

export function rect(x, y, w, h, o = {}) {
  return `<rect x="${n(x)}" y="${n(y)}" width="${n(w)}" height="${n(h)}" fill="${o.fill || 'none'}"${o.opacity !== undefined ? ` opacity="${o.opacity}"` : ''}${o.stroke ? ` stroke="${o.stroke}" stroke-width="${o.width || 1}"` : ''}${o.rx ? ` rx="${o.rx}"` : ''} />`
}

export function path(d, o = {}) {
  return `<path d="${d}" fill="${o.fill || 'none'}"${o.opacity !== undefined ? ` opacity="${o.opacity}"` : ''}${o.stroke ? ` stroke="${o.stroke}" stroke-width="${o.width || 1}"` : ''}${o.cap ? ` stroke-linecap="${o.cap}"` : ''}${o.join ? ` stroke-linejoin="${o.join}"` : ''} />`
}
...
export function svg(w, h, body, o = {}) {
  return [
    `<svg xmlns="http://www.w3.org/2000/svg" width="${w}" height="${h}" viewBox="0 0 ${w} ${h}" font-family="Archivo, system-ui, sans-serif">`,
...
export const SOURCE =
  'English Wikipedia daily pageviews, user agents only, Wikimedia REST API. Every article that ever reached the daily top 1,000, 1 July 2015 to 13 August 2026; the 220 largest single reading days on record.'
